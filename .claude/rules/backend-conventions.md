...
- Keep ground-truth reads as **direct `kysely`**, inline — do NOT wrap them in a general accessor/abstraction. A test oracle must be explicit, independently typed, and coupled to the concrete storage truth on purpose (that's what catches engine-specific bugs; a wrapper would mask them, and a general one just re-creates the Knex-accessor coupling we removed). The *only* extraction worth making is a small, **named** helper for a non-trivial read that repeats verbatim (e.g. an event-join `findOwnerEvent(ownerId, type)`) — thin `kysely` inside, named for intent; inline everything trivial.
