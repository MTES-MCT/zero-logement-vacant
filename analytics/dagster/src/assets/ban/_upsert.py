...
def create_temp_table(cursor) -> None:
    cursor.execute("""
        CREATE TEMP TABLE IF NOT EXISTS temp_ban_addresses (
            ...
            postal_code TEXT, city TEXT, city_code TEXT, latitude FLOAT,
            longitude FLOAT, score FLOAT, ban_id TEXT, address_kind TEXT,
...
def prepare_not_found(
    api_data: pd.DataFrame, address_kind: AddressKind
) -> pd.DataFrame:
...
def copy_upsert(cursor, df: pd.DataFrame) -> int:
    ...
    buf.seek(0)
    cursor.copy_from(
        buf, "temp_ban_addresses", sep="\t", null="NULL", columns=EXPECTED_COLS
    )

    cursor.execute("""
        INSERT INTO ban_addresses (
            ref_id, house_number, address, street, postal_code, city, city_code,
