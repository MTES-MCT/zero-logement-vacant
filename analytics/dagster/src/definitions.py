...
from .assets.dwh.ingest.ingest_lovac_ff_s3_asset import import_cerema_ff_lovac_data_from_s3_to_duckdb
...
daily_update_dwh_job = define_asset_job(
    ...
    selection=AssetSelection.assets(*[*dwh_assets, *dbt_analytics_assets]) - AssetSelection.assets(*[setup_s3_connection, import_cerema_ff_lovac_data_from_s3_to_duckdb])
)

yearly_update_ff_dwh_job = define_asset_job(
    name="datawarehouse_build_ff_data",
    selection=AssetSelection.assets(*[setup_s3_connection, import_cerema_ff_lovac_data_from_s3_to_duckdb])
)

daily_refresh_schedule = ScheduleDefinition(
    job=daily_update_dwh_job,
    cron_schedule="0 0 * * *"
)

yearly_ff_refresh_schedule = ScheduleDefinition(
    job=yearly_update_ff_dwh_job,
