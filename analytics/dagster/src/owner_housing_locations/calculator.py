...
class DistanceCalculator:
    ...
    def _has_coordinates(address_data: tuple | None) -> bool:
        ...
        latitude, longitude = address_data[2], address_data[3]
        return bool(
...
def calculate_owner_housing_locations(
    ...
    if not scope.establishment_id and not scope.geo_codes and not allow_full_year:
        raise ValueError(
            "Refusing an unscoped LOVAC location run without explicit full-year opt-in."
        )
...
def main() -> None:
    ...
    parser.add_argument(
        "--allow-full-year",
        action="store_true",
        help="Explicitly allow a run without establishment or geo-code scope.",
    )
    ...
    try:
        report = calculate_owner_housing_locations(
            ...
            allow_full_year=args.allow_full_year,
            limit=args.limit,
            force=args.force,
