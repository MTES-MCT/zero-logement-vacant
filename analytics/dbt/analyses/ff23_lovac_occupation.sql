...
vacancy_data AS (
    SELECT
        ...
        COALESCE(lovac_2022.debutvacance, lovac_2021.debutvacance, lovac_2020.debutvacance, lovac_2019.debutvacance) AS debut_vacance,
        ...
        (CASE WHEN lovac_2022.mutation_date IS NOT NULL THEN 1 ELSE 0 END +
         ...
         CASE WHEN lovac_2019.mutation_date IS NOT NULL THEN 1 ELSE 0 END) AS mutation_cumul,

        -- Last mutation information (most recent non-null mutation data)
        CASE
            WHEN COALESCE(lovac_2022.mutation_date, lovac_2021.mutation_date, lovac_2020.mutation_date, lovac_2019.mutation_date) IS NOT NULL
            THEN true ELSE false
        END AS last_mutation,

        COALESCE(lovac_2022.mutation_date, lovac_2021.mutation_date, lovac_2020.mutation_date, lovac_2019.mutation_date) AS last_mutation_date,
        COALESCE(lovac_2022.mutation_nature, lovac_2021.mutation_nature) AS last_mutation_nature,
        COALESCE(lovac_2022.mutation_value, lovac_2021.mutation_value)  AS last_mutation_value
    FROM ff
...
WHERE (vacant_lovac22 IS TRUE OR vacant_lovac21 IS TRUE OR vacant_lovac20 IS TRUE OR vacant_lovac19 IS TRUE)
), base_data AS (
    SELECT
        ff.ff_idlocal,
...
SELECT
    ...
    COUNT(*) * 1.0 / NULLIF(SUM(CASE WHEN occupation != 'vacant' THEN 1 ELSE 0 END), 0) AS ratio,
    ...
    SUM(CASE WHEN vacant_lovac22 THEN 1 ELSE 0 END) * 1.0 / NULLIF(COUNT(*), 0) AS from_lovac22_ratio,

    SUM(CASE WHEN vacant_lovac21 THEN 1 ELSE 0 END) AS from_lovac21_total,
    SUM(CASE WHEN vacant_lovac21 THEN 1 ELSE 0 END) * 1.0 / NULLIF(COUNT(*), 0) AS from_lovac21_ratio,

    SUM(CASE WHEN vacant_lovac20 THEN 1 ELSE 0 END) AS from_lovac20_total,
    SUM(CASE WHEN vacant_lovac20 THEN 1 ELSE 0 END) * 1.0 / NULLIF(COUNT(*), 0) AS from_lovac20_ratio,

    SUM(CASE WHEN vacant_lovac19 THEN 1 ELSE 0 END) AS from_lovac19_total,
    SUM(CASE WHEN vacant_lovac19 THEN 1 ELSE 0 END) * 1.0 / NULLIF(COUNT(*), 0) AS from_lovac19_ratio,

    -- Total et ratio des logements ayant une mutation
    SUM(CASE WHEN last_mutation THEN 1 ELSE 0 END) AS mutations_total,
    SUM(CASE WHEN last_mutation THEN 1 ELSE 0 END) * 1.0 / NULLIF(COUNT(*), 0) AS mutations_ratio,

    -- Valeur moyenne et médiane des mutations pour les logements ayant une valeur de mutation non vide
    AVG(last_mutation_value) FILTER(WHERE last_mutation_value IS NOT NULL) AS mutations_value_average,
    PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY last_mutation_value) FILTER(WHERE last_mutation_value IS NOT NULL) AS mutations_value_mediane,
