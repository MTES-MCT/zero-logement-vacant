WITH ff_data AS (
    SELECT
        ...
        ST_X(
            ST_TRANSFORM(
                ST_GEOMFROMWKB(FROM_HEX(ff_geomloc)), 'EPSG:2154', 'EPSG:4326'
            )
        ) AS latitude,
        ST_Y(
            ST_TRANSFORM(
...
clean_data AS (
    ...
    SELECT
        ...
        fd.cadastral_classification,
        COALESCE(l24.latitude, l23.latitude, fd.latitude) AS latitude,
        COALESCE(l24.longitude, l23.longitude, fd.longitude) AS longitude,
        COALESCE(
            l24.last_mutation_date,
            l23.last_mutation_date,
            fd.last_mutation_date
        ) AS last_mutation_date,
        COALESCE(l24.last_transaction_date, l23.last_transaction_date)
            AS last_transaction_date,
        COALESCE(l24.last_transaction_value, l23.last_transaction_value)
            AS last_transaction_value,
        COALESCE(l24.occupancy_history, l23.occupancy_history)
            AS occupancy_history
    FROM ff_data fd
...
SELECT
    ...
    CASE
        WHEN
            TRY_CAST(
                TRY_STRPTIME(last_mutation_date, '%d%m%Y') AS DATE
            ) IS NOT NULL
            THEN
