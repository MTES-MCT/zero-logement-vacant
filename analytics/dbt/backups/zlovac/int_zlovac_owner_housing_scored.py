...
def _add_fuzzy_ratio(df_owners):
    ...
    df_owners["match_raw_address"] = df_owners.apply(
        lambda x: match(x["ff_owner_raw_address"], x["owner_raw_address"]), axis=1
    )

    df_owners["match_postal_code"] = (
        df_owners["ff_owner_postal_code"] == df_owners["owner_postal_code"]
    )
    ...
    df_owners["match_fullname"] = df_owners.apply(
        lambda x: match(x["ff_owner_fullname"], x["owner_fullname"]), axis=1
    )
    return df_owners
...
def _add_first_name_ratio(df_owners, df_prenoms):
    ...
    df_owners["owner_first_names"] = (
        df_owners["owner_fullname"].fillna("").apply(keyword_processor.extract_keywords)
    )
    df_owners["ff_owner_first_names"] = (
        df_owners["ff_owner_fullname"]
        .fillna("")
        .apply(keyword_processor.extract_keywords)
    )
    df_owners["match_same_first_name"] = df_owners.apply(
        lambda x: len(
            set(x["owner_first_names"]).intersection(set(x["ff_owner_first_names"]))
        )
        >= 1,
        axis=1,
    )
...
def _get_df_with_no_scores(df_owners):
    ...
    df_with_no_match = df_owners.merge(
        df_owners.groupby("local_id")["final_owner_score"]
        .sum()
        .reset_index()
        .query("final_owner_score == 0")
        .drop("final_owner_score", axis=1),
        on="local_id",
        how="inner",
    )

    df_with_no_match["final_owner_score"] = df_with_no_match.apply(
        lambda x: _process_biscom_scores(x)[0], axis=1
    )
    df_with_no_match["final_owner_reason"] = df_with_no_match.apply(
        lambda x: _process_biscom_scores(x)[1], axis=1
    )
...
def _process_scores(df_owners):
    df_owners["final_owner_score"] = df_owners.apply(
        lambda x: _define_score_reason(x)[0], axis=1
    )
    df_owners["final_owner_reason"] = df_owners.apply(
        lambda x: _define_score_reason(x)[1], axis=1
    )
...
def _structure_data(df):
    ...
    for index in range(1, 7):
        data.append(
            ...
            .dropna(
                subset=["ff_owner_fullname"]
            )  # Drop toutes les lignes ou le nom est vide
        )
    df = pd.concat(data)
    return df
...
def _reprocess_rank(df_owners):
    ...
    df_owners_for_processing_rank = df_owners[
        ["local_id", "rank", "final_owner_score"]
    ].copy()
    df_owners_for_processing_rank["new_rank"] = df_owners_for_processing_rank.groupby(
        "local_id"
    )["final_owner_score"].rank(ascending=False, method="first")
    assert (
        df_owners[["local_id", "rank", "final_owner_score"]].duplicated().sum() == 0
    ), "There are duplicates in the dataframe df_owners. The rank reprocessing will not work properly."
    assert (
        df_owners_for_processing_rank[["local_id", "rank", "final_owner_score"]]
        .duplicated()
        .sum()
        == 0
    ), "There are duplicates in the dataframe df_owners_for_processing_rank. The rank reprocessing will not work properly."

    df_owners = df_owners.merge(
        df_owners_for_processing_rank,
...
def model(dbt, session):
    ...
    assert (
        df_owners[["local_id"]].duplicated().sum() == 0
    ), "There are duplicates in the dataframe df_owners"
    ...
    assert (
        df_owners.shape[0] > 0
    ), "The dataframe df_owners is empty. The structure_data function is not working properly."
    assert (
        df_owners[["local_id", "rank"]].duplicated().sum() == 0
    ), "There are duplicates in the dataframe df_owners at the generation"
