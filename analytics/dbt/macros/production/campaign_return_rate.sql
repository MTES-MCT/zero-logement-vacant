{% macro process_return_rate_for_campaigns(n_month, check_next_campaign=False) %}
    MAX(CASE
    ...
    (COUNT(DISTINCT CASE
        ...
        {% endif %}
        THEN pch.housing_id
    END) * 100.0 / NULLIF(COUNT(DISTINCT h.id), 0)) AS return_rate_{{ n_month }}{% if not check_next_campaign %}_no_filter{% endif %}
{% endmacro %}
