...
-- delete from housing h where h.local_id in (select ff_idlocal from _extract_zlv) and h.local_id not in  (select ff_idlocal from _extract_zlv_);
...
insert into owners(full_name, administrator, raw_address, birth_date, owner_kind, owner_kind_detail, local_ids)
    select
           ...
           (case
               when ff_jdatnss <> '0' and ff_jdatnss <> '00/00/0000' and (
                   ...
                   (var.owner like '%' || split_part(split_part(trim(nom_ff), '/', 2), ' ', 1) || '%')) then to_date('1899-12-30', 'YYYY-MM-DD') + interval '1 day' * to_number(ff_jdatnss, '99999') end) as _birth_date,
           (case
    ...
    group by (owner, administrator, _birth_date, adresse1, adresse2, adresse3, adresse4, nom_ff, owner_kind, owner_kind_detail);
...
select o.id, h.id from housing h, owners o, _extract_zlv_ e
where h.invariant = e.invariant
  and h.local_id = any(o.local_ids)
  and h.local_id = ff_idlocal
  and (h.building_id = ff_idbat or h.building_id is null)
  and h.raw_address = array[ltrim(trim(libvoie), '0'), trim(libcom)]
  and h.insee_code = codecom
  and h.latitude = ff_x_4326
--   and (h.latitude = replace(ff_x_4326, ',', '.')::double precision or h.latitude is null)
  and h.longitude = ff_y_4326
--   and (h.longitude = replace(ff_y_4326, ',', '.')::double precision or h.longitude is null)
...
--   and h.building_year = (case when (ff_jannath > 100) then ff_jannath end)
  and (h.mutation_date = to_date(anmutation, 'M/D/YY') or mutation_date is null)
  and (h.taxed = (trim(txtlv) <> '') or taxed is null)
