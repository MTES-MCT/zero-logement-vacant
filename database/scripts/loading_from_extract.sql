...
select
       ...
       (case when ff_jdatnss <> 0 then to_date('1899-12-30', 'YYYY-MM-DD') + interval '1 day' * ff_jdatnss end),
       (case
           when trim(groupe) = '' then 'Particulier'
           when not('%' || var.owner || '%' like '%' || trim(nom_ff) || '%') and
                not('%' || trim(nom_ff) || '%' like '%' || var.owner || '%') then 'Autre'
       ...
       (case
           when ('%' || var.owner || '%' like '%' || trim(nom_ff) || '%') or
                ('%' || trim(nom_ff) || '%' like '%' || var.owner || '%') then ff_ndroit end),
       array_agg(distinct(invariant))
from extract_zlv, lateral (
