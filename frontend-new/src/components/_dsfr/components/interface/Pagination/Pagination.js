...
const Pagination = ({
  ...
  const surrendingRight = getSurrendingRight(
    currentPage,
    surrendingPages,
    pageCount
  );
  ...
  return (
    ...
    >
      <ul className="fr-pagination__list">
        <li>{getItem(true, 1, firstAria, 'first')}</li>
        <li>{getItem(true, currentPage - 1, prevAria, 'prev', prevLabel)}</li>
        <PaginationItem
          isVisibleOnMobile
          page={1}
          anchorAs={anchorAs}
          ...
          aria={pageAria(1)}
        />
        {surrendingLeft.hasEllipsis && <li>…</li>}
        ...
        {surrendingRight.hasEllipsis && <li>…</li>}
        {pageCount !== 1 && (
          <PaginationItem
        ...
        <li>{getItem(false, currentPage + 1, nextAria, 'next', nextLabel)}</li>
        <li>{getItem(false, pageCount, lastAria, 'last')}</li>
      </ul>
    </nav>
  );
};
...
Pagination.propTypes = {
  ...
  buildURL: (props, propName, componentName) => {
    ...
    if ((!buildURL && !onClick) || (buildURL && onClick)) {
      return new Error(
        `You must specify only one of props 'buildURL' or 'onClick' in '${componentName}'.`
      );
    }
    if (buildURL) {
