...
const ModalDialog = ({
  ...
  const title = Children.toArray(children).filter(
    (child) => child.props.__TYPE === 'ModalTitle'
  );
  const content = Children.toArray(children).filter(
    (child) => child.props.__TYPE === 'ModalContent'
  );
  const footer = Children.toArray(children).filter(
    (child) => child.props.__TYPE === 'ModalFooter'
  );
  const close = Children.toArray(children).filter(
    (child) => child.props.__TYPE === 'ModalClose'
  );
  ...
  const handleOverlayClick = (e) => {
    ...
    if (
      ...
      e.target.className.indexOf('closing-overlay') > -1
    ) {
