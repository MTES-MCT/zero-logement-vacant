...
const CampaignCreationModal = (
    ...
    const schema = yup.object().shape(isReminder ? {} : {
                ...
                <Container fluid>
                    <Text size="md">
                        <span data-testid="housing-infos">
                            Vous êtes sur le point de créer une campagne comportant <b>{displayCount(housingCount, 'logement')}.</b>
                        </span>
                    </Text>
                    {!isReminder &&
                        <Row gutters>
                            ...
                            {housingExcudedCount !== undefined && housingExcudedCount > 0 &&
                                <>
                                    {housingExcudedCount === 1 ?
                                        <i>Un logement a été retiré des résultats de la
                                            recherche{hasFilters(filters) && <> avec ces filtres</>}.</i> :
                                        <i>{housingExcudedCount} logements ont été retirés des résultats de la
                                            recherche{hasFilters(filters) && <> avec ces filtres</>}.</i>
                                    }
                                </>
                            }
