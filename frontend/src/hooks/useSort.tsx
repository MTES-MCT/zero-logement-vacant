...
export function useSort<Sortable extends object>(options?: UseSortOptions<Sortable>) {
  const [sort, setSort] = useState<Sort<Sortable>>({})

  function getIcon(key: keyof Sortable): JSX.Element {
    const direction = sort?.[key]
    return direction
      ? <Icon name={direction === 'asc' ? 'ri-arrow-up-line' : 'ri-arrow-down-line' } color="var(--text-title-grey)" />
