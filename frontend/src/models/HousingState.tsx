...
): HousingStatusPrecision | undefined => {
  return getSubStatus(status, subStatusTitle)?.precisions?.filter(
    (p) => p.title === precisionTitle
  )[0];
};
...
export const getStatusPrecisionOptions = (
  status: HousingStatus,
  subStatus?: string
) => {
  const housingSubStatus = getHousingState(status).subStatusList?.find(
    (s) => s.title === subStatus
  );
  return housingSubStatus?.precisions?.map((subStatus) => ({
    value: subStatus.title,
    label: subStatus.title,
  }));
};
