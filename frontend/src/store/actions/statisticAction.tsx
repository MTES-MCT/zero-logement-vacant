...
export const CONTACTED_HOUSING_COUNT_FETCHED =
...
export const HOUSING_IN_PROGRESS_WITH_SUPPORT =
  'HOUSING_IN_PROGRESS_WITH_SUPPORT';
export const HOUSING_IN_PROGRESS_WITHOUT_SUPPORT =
...
export const getStatistics = () => {
  return function (dispatch: Dispatch) {
    ...
    Promise.all([
      monitoringService.getEstablishmentsCount().then((count) => {
        dispatch({
          type: ESTABLISHMENTS_COUNT_FETCHED,
          count,
        });
      }),
      monitoringService.getContactedHousingCount().then((count) => {
        dispatch({
          type: CONTACTED_HOUSING_COUNT_FETCHED,
          count,
        });
      }),
      monitoringService.getWaitingHousingCount().then((count) => {
        dispatch({
          type: WAITING_HOUSING_COUNT_FETCHED,
          count,
        });
      }),
      monitoringService.getAnswersCount().then((count) => {
        dispatch({
          type: ANSWERS_COUNT_FETCHED,
          count,
        });
      }),
      monitoringService.getHousingInProgressWithSupportCount().then((count) => {
        dispatch({
      ...
      monitoringService.getHousingExitWithSupportCount().then((count) => {
        dispatch({
          type: HOUSING_EXIT_WITH_SUPPORT,
          count,
        });
      }),
      monitoringService.getHousingExitWithoutSupportCount().then((count) => {
        dispatch({
