...
const MonitoringView = () => {
  ...
  useEffect(() => {
    ...
    if (
      paginatedHousingToContactFilters.establishmentIds?.indexOf(
        establishmentId
      ) === -1
    ) {
  ...
  useEffect(() => {
    if (establishmentData) {
      setEstablishmentDetailData(
        establishmentData.find((_) => _.id === establishmentId)
      );
    }
    ...
    render: ({ lastContact }: Housing) =>
      lastContact && (
        <>
          ...
          <br />(
          {differenceInDays(new Date(), lastContact)
            ? 'il y a ' +
              displayCount(
                differenceInDays(new Date(), lastContact),
                'jour',
        ...
        <ul>
          ...
          <li>
            ...
            {establishmentDetailData?.firstActivatedAt && (
              <>
                {format(establishmentDetailData.firstActivatedAt, 'dd/MM/yyyy')}{' '}
                (
                {differenceInDays(
                  new Date(),
                  establishmentDetailData?.firstActivatedAt
          ...
          <li>
            <b>
              Nombre de dossiers mis à jour dans les 30 derniers jours :&nbsp;
            </b>
          ...
          <li>
            ...
            {establishmentDetailData?.contactedHousingCount} (
            {percent(
              establishmentDetailData?.contactedHousingCount,
              establishmentDetailData?.housingCount
            )}
          ...
          <li>
            ...
            {Math.floor(
              establishmentDetailData?.contactedHousingPerCampaign ?? 0
            )}{' '}
            (
            {percent(
              establishmentDetailData?.contactedHousingPerCampaign,
              establishmentDetailData?.housingCount
            )}
            %)
          </li>
          ...
          <li>
            <b>
              Temps d'envoi de la première campagne après inscription :&nbsp;
            </b>
            {establishmentDetailData?.firstActivatedAt &&
              establishmentDetailData?.firstCampaignSendingDate && (
                <>
                  {differenceInDays(
                    establishmentDetailData?.firstCampaignSendingDate,
                    establishmentDetailData?.firstActivatedAt
          ...
          <>
            ...
            <Text>
              <b>
                ...
                dans le sous-statut "{FirstContactToContactedSubStatus}"
              </b>
            </Text>

            <HousingList
              paginatedHousing={paginatedHousingToContact}
