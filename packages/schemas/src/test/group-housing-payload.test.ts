...
describe('Group housing payload', () => {
  test.prop<GroupAddHousingPayload>({
    all: fc.boolean(),
    ...
    const validate = () => groupHousingPayload.validateSync(payload);

    expect(validate).not.toThrow();
  });

  it('should reject when "all" is missing', () => {
    const payload = {
      ids: [],
      filters: {}
    };
    ...
    expect(validate).toThrow(/all/i);
  });

  it('should reject when "ids" contains a non-UUID', () => {
    const payload = {
      all: false,
      ids: ['not-a-uuid'],
      filters: {}
    ...
    expect(validate).toThrow(/uuid/i);
  });
});
