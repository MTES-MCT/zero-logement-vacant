...
const YEAR = '2023';
const BAN_FILE_LOCATION = '/Volumes/Zéro_Logement_Vacant/BAN/adresses-france-BAL.csv';
const result = execSync(`wc -l < "${BAN_FILE_LOCATION}"`, { encoding: 'utf-8' });
    ...
    const doProcess = async () => {
      ...
      if(ban_id) {
        ...
        if(banItem?.long && banItem?.lat) {
          await db.raw(`update fast_housing set longitude='${banItem?.long}', latitude='${banItem?.lat}' where local_id='${housing.local_id}'`);
        }
      }
      resolve();
    }
...
function transform(transformFunction: { (row: any, callback: any): void; (arg0: any, arg1: any): void; }) {
  let isFirstCol = true;
  return require('stream').Transform({
    objectMode: true,
    transform: function(row: any, encoding: any, callback: (arg0: null, arg1: any) => any) {
      if (isFirstCol) {
        isFirstCol = false;
...
const main = async () => {
  ...
  .pipe(transform(function(row: { [s: string]: unknown; } | ArrayLike<unknown>, callback: (arg0: null, arg1: unknown[]) => void) {
  ...
  .on('end', async () => {
    ...
    const count = await db.raw(`select count(*) from fast_housing where latitude is null`);

    progressBarUpdate.start(parseInt(count.rows[0].count), 0);

    const queryStream = db.raw(`SELECT local_id, ban_id, idpar
    FROM fast_housing fh
    JOIN df_housing_nat_${YEAR} df ON fh.local_id = df.idLocal
    WHERE fh.latitude IS NULL`).stream();
