...
describe('Housing controller', () => {
    ...
    describe('updateHousing', () => {
        ...
        it('should be forbidden for a not authenticated user', async () => {
            await request(app).post(testRoute(Housing1.id)).expect(constants.HTTP_STATUS_UNAUTHORIZED);
        })

        it('should received a valid housingId', async () => {

            await withAccessToken(
                request(app).post(testRoute(randomstring.generate()))
                    .send(validBody)
        ...
        it('should received a valid housingUpdate object', async () => {
            ...
            await withAccessToken(
                request(app).post(testRoute(Housing1.id))
                    .send({ ...validBody, housingUpdate: undefined })
            ).expect(constants.HTTP_STATUS_BAD_REQUEST)

            await withAccessToken(
                request(app).post(testRoute(Housing1.id))
                    .send({ ...validBody, housingUpdate: {...validBody.housingUpdate, status: undefined} })
            ).expect(constants.HTTP_STATUS_BAD_REQUEST)

            await withAccessToken(
                request(app).post(testRoute(Housing1.id))
                    .send({ ...validBody, housingUpdate: {...validBody.housingUpdate, status: randomstring.generate()} })
            ).expect(constants.HTTP_STATUS_BAD_REQUEST)

            await withAccessToken(
                request(app).post(testRoute(Housing1.id))
                    .send({ ...validBody, housingUpdate: {...validBody.housingUpdate, contactKind: undefined} })
            ).expect(constants.HTTP_STATUS_BAD_REQUEST)
            ...
            await db(eventsTable)
                ...
                .then(result => expect(result).toMatchObject(
                    expect.objectContaining({
                        ...
                        created_by: User1.id,
                        content: 'Passage à Suivi en cours. ' + validBody.housingUpdate.comment
                    })
                ));
        ...
        it('should remove housing from campaign when updating to status NeverContacted', async () => {
            ...
            await withAccessToken(
                request(app).post(testRoute(Housing1.id))
                    .send({ ...validBody, housingUpdate: {...validBody.housingUpdate, status: HousingStatusApi.NeverContacted} })
            ).expect(constants.HTTP_STATUS_OK)
