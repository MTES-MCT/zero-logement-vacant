...
export async function seed(knex: Knex): Promise<void> {
  ...
  const addresses = await ban
    ...
    .catch((error) => {
      console.warn(
        'BAN reverse geocoding failed (API may be unavailable):',
        error.message
      );
      console.warn('Continuing without owner addresses...');
      return [];
    });
  ...
  if (addresses.length > 0) {
    ...
    console.log('No addresses to insert (BAN API unavailable).');
  }
}
