...
class LovacUsersExporter:
    ...
    def fetch_all_pages(self, endpoint: str, desc: str = "Fetching") -> List[Dict]:
        ...
        while url:
            ...
            try:
                # Disable auto-redirect to avoid server corruption adding %23
                response = self.session.get(url, timeout=60, allow_redirects=False)
                # Handle redirect manually if needed
                if response.status_code in (301, 302, 303, 307, 308):
                    redirect_url = response.headers.get("Location", "")
                    redirect_url = redirect_url.replace("%23", "").replace("#", "")
                    # Handle relative URLs
                    if redirect_url.startswith("/"):
                        redirect_url = "https://portaildf.cerema.fr" + redirect_url
                    response = self.session.get(redirect_url, timeout=60, allow_redirects=False)
                ...
                # Fix: API returns http:// but server redirects to https:// with corruption
                # Force https and remove %23 (encoded #) that appears during redirect
                if next_url:
                    url = next_url.replace("http://", "https://").replace("%23", "").replace("#", "")
                else:
                    url = None
                page += 1
