...
export const getLocalitiesGeocode = async (
  perimeter: PerimeterType
): Promise<string[]> => {
  ...
  for (const key in perimeter.reg_complet) {
    const geoCodes = await db
      .table('_localities')
      .select('codgeo')
      .where('reg', perimeter.reg_complet[key]);
    flattenedArray = [
      ...flattenedArray,
      ...geoCodes.map((localities: { codgeo: string }) => localities.codgeo)
    ];
  }

  for (const key in perimeter.dep_complet) {
    const geoCodes = await db.raw(
      `select codgeo from _localities where dep='${perimeter.dep_complet[key]}'`
    );
    flattenedArray = [
      ...flattenedArray,
      ...geoCodes.rows.map(
        (localities: { codgeo: string }) => localities.codgeo
      )
    ];
  }
...
export async function structureToEstablishment(
  structure: Structure
): Promise<EstablishmentDBO> {
  return {
    id: uuidv4(),
    name: structure.name,
    siren: Number(structure.siret.substring(0, 9)),
    available: true,
    localities_geo_code: structure.perimeter
      ? await getLocalitiesGeocode(structure.perimeter)
      : [],
    kind: structure.kind,
    source: 'cerema',
    updated_at: new Date()
...
export const getTestStructure = (id: number): Structure => {
  const structure = TEST_STRUCTURES.find(
    (structure) => structure.establishmentId === id
  );
